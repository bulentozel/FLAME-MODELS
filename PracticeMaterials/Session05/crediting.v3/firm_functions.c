...
int firm_check_loan_status()
{
    ...
    if (amount > 0) {
        DEBTS += amount;
        printf("Firm ID = %d has gotten %f amount of loan from Bank ID = %d \n", ID, amount, bank);
    }
    else {
        printf("Firm ID = %d is bankrupt and is removed from the economy!!! \n", ID);
        return 1; /* Returning 1 means the agent is removed */
    }
    return 0; /* Returning zero means the agent is not removed */
}
